...
const FALLBACK_NAME = "Commitgen";

// Environment variables
const APP_NAME = process.env.APP_NAME || FALLBACK_NAME;
...
const DEFAULT_MAIL_PROVIDER = process.env.EMAIL_PROVIDER || "nodemailer"; // 'nodemailer', 'zeptomail', or 'resend'
...
class NodemailerProvider implements EmailProviderInterface {
  ...
  constructor(config: {
    ...
    this.defaultFromEmail =
      config.defaultFromEmail || MAIL_USER || "hello@example.com";
...
class ZeptoMailProvider implements EmailProviderInterface {
  private client: SendMailClient; // ZeptoMail client
  ...
  constructor(config: {
    ...
    this.apiUrl = config.apiUrl || "api.zeptomail.com/";
    this.defaultFromEmail = config.defaultFromEmail || "hello@visitsmb.com";
    this.defaultFromName =
      config.defaultFromName || APP_NAME || "Notification Service";
  ...
  async sendEmail(options: EmailOptions): Promise<EmailResult> {
    try {
      ...
      return {
        success: true,
        messageId:
          response?.messageId || response?.data?.request_id || undefined,
        details: response,
      };
    } catch (error: unknown) {
      logger.error("Failed to send email via ZeptoMail:", error);
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
      return {
        success: false,
    ...
    try {
      ...
      if (options.templateId) {
        ...
        const response = await this.client.sendMailWithTemplate(
          templatePayload as unknown as Record<string, unknown>
        );

        return {
          success: true,
          messageId: response?.data?.request_id || undefined,
          details: response,
        };
      }
...
class ResendProvider implements EmailProviderInterface {
  ...
  constructor(config: {
    ...
    this.defaultFromEmail = config.defaultFromEmail || "hello@example.com";
    this.defaultFromName =
      config.defaultFromName || APP_NAME || "Notification Service";
  ...
  async sendEmail(options: EmailOptions): Promise<EmailResult> {
    try {
      ...
      const success = Boolean(response && response.data?.id);
  ...
  async sendTemplateEmail(
    ...
    try {
      if (options.templateContent) {
        ...
        // We cast to any here because the Resend types might not fully support this specific usage pattern
        // or we are mocking the React element structure.
...
export class EmailService {
  ...
  constructor(
    providerType:
      ...
      | "zeptomail"
      | "resend"
      | string = DEFAULT_MAIL_PROVIDER,
    config: Record<string, unknown> = {}
  ) {
    this.defaultFromEmail =
      (config.defaultFromEmail as string) || MAIL_USER || "hello@visitsmb.com";
    this.defaultFromName =
          ...
          ${socialLinks
            .map(
              (link) =>
                `<a href="${
                  link.url
                }" target="_blank" style="display: inline-block; margin: 0 8px; color: #1a74e4; text-decoration: none;">${
                  link.name
                }</a>${
      ...
      <div style="text-align: left; padding: 40px; background-color: #ffffff; color: #1f1b4a; font-family: Arial, sans-serif; font-size: 14px; line-height: 1.5;">
        ...
        <div style="width: 100%; margin-bottom: 20px;">
          <div style="width: 48px; height: 48px; display: inline-block; vertical-align: middle;">
            <img src="${MAIL_LOGO || "https://via.placeholder.com/48"}" alt="${
      APP_NAME || "Logo"
    }" width="48" height="48" />
          </div>
  ...
  <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #334155; line-height: 1.5; background-color: #f0f9ff;">
    <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
      ...
      <div style="background-color: #ffffff; border-radius: 24px; padding: 40px; margin-bottom: 24px; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.03);">
        <h2 style="color: #0c4a6e; font-size: 24px; font-weight: 600; margin-top: 0; margin-bottom: 20px;">${
          title || "Notification"
        }</h2>
  ...
  generateSimpleMinimalistTemplate(title: string, content: string): string {
    ...
    <div style="text-align: left; padding: 40px; background-color: #ffffff; color: #334155; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size: 15px; line-height: 1.6; border-radius: 24px; max-width: 600px; margin: 0 auto; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.03);">
      ...
      APP_SUPPORT_MAIL || "support@example.com"
        ...
        <p style="margin-bottom: 0;">&copy; ${new Date().getFullYear()} ${
      APP_NAME || "Application"
    }. All rights reserved.</p>
      </div>
    </div>
